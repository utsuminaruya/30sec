...
import type { Answers, Plan, Job, Task, Guide } from '../../../types';
...
export async function POST(req: Request) {
  ...
  await supabaseAdmin.from('responses').insert({
    ...
    nihongo: body.nihongo,
    care_exp: body.care_exp,
  ...
  const jobs: Job[] = (jobsRaw ?? []).slice(0, 3) as Job[];

  // 3) 学習タスク
  const taskKeys = pickTasks(body.nihongo);
  const { data: tasksRes, error: tasksErr } = await supabaseAdmin
    .from('learning_tasks')
    .select('*')
    .in('key', taskKeys)
    .limit(3);
  ...
  const tasks: Task[] = (tasksRes ?? []).slice(0, 3) as Task[];

  // 4) 生活ガイド
  let guide: Guide | null = null;
  if (body.prefecture !== 'other') {
    const { data: guidesRes, error: guidesErr } = await supabaseAdmin
      .from('lifestyle_guides')
      .select('*')
      .eq('prefecture', body.prefecture)
      .limit(1);

    if (guidesErr) {
      return NextResponse.json({ error: guidesErr.message }, { status: 500 });
    }
    guide = (guidesRes && guidesRes[0]) ? (guidesRes[0] as Guide) : null;
  }

  const plan: Plan = { jobs, tasks, guide };
